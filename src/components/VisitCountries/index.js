...
class VisitCountries extends Component {
  ...
  onClickRemove = id => {
    ...
    const removeList = countriesList.map(each => {
      if (each.id === id) {
    ...
    this.setState({countriesList: removeList})
  }
    ...
    return (
      <AppContainer>
        ...
        <CountryListContainer>
          {countriesList.map(each => (
            <CountryList
              key={each.id}
              countryDetails={each}
              onCLickVisited={this.onClickVisit}
            />
          ))}
        </CountryListContainer>
        ...
        {updatedList.length > 0 ? (
          ...
          <NoVisitedCountry>
            <NoCountriesFound>No Countries Visited Yet</NoCountriesFound>
          </NoVisitedCountry>
        )}
      </AppContainer>
    )
